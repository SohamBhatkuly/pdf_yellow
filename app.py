...
# Matches stroke-color operators: RG (rgb), G (gray), K (cmyk), SC/SCN (other spaces)
STROKE_RE = re.compile(rb"(?<![\w.])(?:-?[\d.]+\s+){1,4}(?:RG|G|K|SCN?)(?![\w])")


def recolor_doc_strokes(d, rgb):
    """Rewrite stroke colors directly in the PDF content streams."""
    color_op = ("%.3f %.3f %.3f RG\n" % tuple(rgb)).encode()
    for page in d:
        page.clean_contents()  # merge/normalize content streams
        for xref in page.get_contents():
            stream = d.xref_stream(xref)
            # prepend so lines with no explicit color (default black) are covered too
            d.update_stream(xref, color_op + STROKE_RE.sub(color_op, stream))
        for xref, *_ in page.get_xobjects():  # form XObjects (common in CAD exports)
            stream = d.xref_stream(xref)
            d.update_stream(xref, STROKE_RE.sub(color_op, stream))
...
if uploaded_file is not None:
    ...
    if not selected_indices:
        st.warning("No valid pages selected.")
        st.stop()
    ...
    col1.image(orig_img, caption=f"Original (Page {preview_page_num})", width="stretch")
    ...
    if engine_mode == "Raster (Image-Based)":
        ...
        # Vector mode preview: temporary single-page doc, strokes rewritten in the content stream
        temp_doc = fitz.open()
        temp_doc.insert_pdf(doc, from_page=preview_idx, to_page=preview_idx)
        recolor_doc_strokes(temp_doc, FLOAT_LINE_COLOR)
    ...
    col2.image(recolored_preview_img, caption=f"Recolored Preview ({engine_mode.split()[0]} Mode)", width="stretch")
    ...
    if st.button("Process & Download PDF"):
        with st.spinner("Processing selected pages..."):
            ...
            if engine_mode == "Vector (Native PDF Shapes)":
                ...
                recolor_doc_strokes(output_doc, FLOAT_LINE_COLOR)
